...
/**
 * Base client using JSON Web Tokens
 *
...
class FBJWTClient {
   ...
   *
   **/
  getEndpointUrl (urlPattern, keys) {
   ...
   *
   **/
  getRequestOptions (urlPattern, urlKeys, payload) {
    const accessToken = this.generateAccessToken(payload)
    const url = this.getEndpointUrl(urlPattern, urlKeys)
    const json = payload ? {payload} : true
  ...
  sendGet (urlPattern, urlKeys) {
    const client = this
    const options = this.getRequestOptions(urlPattern, urlKeys)
  ...
  sendPost (urlPattern, urlKeys, payload) {
    const client = this
    const options = this.getRequestOptions(urlPattern, urlKeys, payload)
