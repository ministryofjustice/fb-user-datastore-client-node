...
const userDataStoreClient = proxyquire('./client', {
...
// Test client in itialised state
// execute these after testing unitialised  tests
const initialisedTests = () => {
  // initialise client
  userDataStoreClient.init(serviceSlug, serviceToken, userDataStoreUrl)

  // Endpoint URLs
  test('When asking for endpoint urls', t => {
    const getUrl =
  userDataStoreClient.getUserEndpointUrl('get', userId)
    t.equal(getUrl, getEndpointUrl, 'it should return the correct value for the get endpoint')
    const setUrl =
  userDataStoreClient.getUserEndpointUrl('set', userId)
  ...
  test('When decrypting the user’s data', async t => {
    const decryptedPayload = userDataStoreClient.decryptPayload(encryptedPayload, userToken)
    t.deepEqual(payload, decryptedPayload, 'it should return the correct payload from valid enctypted input')

    const invalidPayload = userDataStoreClient.decryptPayload('gibberish', userToken)
    t.equal(invalidPayload, undefined, 'it should return the correct payload from valid enctypted input')
  ...
  test('When encrypting the user’s data', async t => {
    const encryptedPayload = userDataStoreClient.encryptPayload(payload, userToken)
    const decryptedPayload = userDataStoreClient.decryptPayload(encryptedPayload, userToken)
    ...
    const encryptedPayloadAgain = userDataStoreClient.encryptPayload(payload, userToken)
  ...
  test('When requesting user data that exists with a valid key', async t => {
    ...
    const decryptedPayload = await userDataStoreClient.get(userId, userToken)
  ...
  test('When updating user data', async t => {
    ...
    const responseBody = await userDataStoreClient.set(userId, userToken, payload)

    const callArgs = stubRequest.getCall(0).args[0]
    t.equal(callArgs.url, setEndpointUrl, 'it should call the correct url')
    t.equal(callArgs.form.payload, encryptedPayload, 'it should post the correct payload')
  ...
  const testError = async (clientMethod, stubMethod, t, requestErrorCode, applicationErrorCode, expectedRequestErrorCode) => {
    ...
    try {
      ...
      t.equal(e.code, applicationErrorCode, 'it should return correct error code')
      t.equal(e.message, expectedRequestErrorCode, `it should return the correct error message (${applicationErrorCode})`)
    }
  ...
  const testSetError = async (t, requestErrorCode, applicationErrorCode, expectedRequestErrorCode) => {
    const clientMethod = async () => {
      return userDataStoreClient.set(userId, userToken, payload)
    }
