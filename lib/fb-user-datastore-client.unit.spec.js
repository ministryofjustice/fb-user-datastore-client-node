...
test('When requesting user data that exists with a valid key', async t => {
  ...
  t.deepEqual(callArgs[0], '/service/:serviceSlug/user/:userId', 'it should pass the correct url pattern')
  t.deepEqual(callArgs[1], {serviceSlug: 'testServiceSlug', userId: 'testUserId'}, 'it should pass the correct data to insert into the url')
...
test('When requesting user data results in an error', async t => {
  ...
  try {
    await userDataStoreClient.getData(userId, userToken)
  } catch (e) {
...
test('When updating user data results in an error', async t => {
  ...
  try {
    await userDataStoreClient.setData(userId, userToken, payload)
  } catch (e) {
    t.deepEqual(e.name, 'Error', 'it should return the correct error type')
    t.deepEqual(e.message, 'boom', 'it should return the correct error message')
  }
